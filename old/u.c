...
static int regex_compiler(regex_t *reg, string pattern, int flags);
static char* regex_matcher(regex_t *reg, string line, regmatch_t *match, int *start); 
...
char** parser_reader(string line, string pattern, int *amnt_values) {
    ...
    *amnt_values = 0;
    ...
    do { // Copies values that match
        values = realloc(values, sizeof(string ) * (*amnt_values + 1));
...
*/
FILE* open_file(string file_path, string flag) {
...
int read_line_num(FILE *stream) {
    string tmp_num = read_line(stream);
